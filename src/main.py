...
class HamAlertClient:
    ...
    def __init__(self):
        ...
        self.modes = [
            m.strip().upper() for m in cfg.get("mode", "FT8").split(",") if m.strip()
        ]
        self._sock = None
    ...
    def _render(self, spots: list, last_updated: str, first_draw: bool) -> None:
        ...
        now = datetime.now()
        recent = [
            txt for ts, txt in spots if (now - ts).total_seconds() < self.SPOT_MAX_AGE
        ][: self.DISPLAY_ROWS]

        header = (
            f" {self.callsign} @ {self.host}:{self.port}   last updated: {last_updated}"
        )
        sep = "\u2500" * self.WIDTH
        footer = " Ctrl-C to quit"

        total_lines = 4 + self.DISPLAY_ROWS  # header + sep + spots + sep + footer
